...
class _YandexMapTestState extends State<YandexMapTest> {
    ...
    setState(() {
      ...
      jsonList.forEach((item) async {
        ...
        var placemark = PlacemarkMapObject(
            mapId: mapObjectId,
            point: Point(latitude: double.parse(jsonList[jsonList.indexOf(item)]['latitude']),
                longitude: double.parse(jsonList[jsonList.indexOf(item)]['longitude'])),
            opacity: 200,
                ...
                child: ListView(
                    children: [
                      ...
                      FloatingActionButton(
                        child: const Text('SEND', textAlign: TextAlign.center, textScaleFactor: 0.9),
                        onPressed: () async {
                          var dio = Dio();
                          var response = await dio.post("http://192.168.1.15:8080/comment",
                              data: {'comment': commentController.text, 'latitude' : point.latitude.toString()});
                          Navigator.popUntil(
                            context,
                            ModalRoute.withName('/'),
                          );
                ...
                children:[
                  ...
                  Row(
                    children: [
                      ...
                      Padding(padding: EdgeInsets.only(left: 160, top: 20, right: 0)),
                      TextButton(
            ...
            children:[
              ...
              SizedBox(
                ...
                child: FloatingActionButton(
                    ...
                    child: const Text('ADD COMMENT', textAlign: TextAlign.center, textScaleFactor: 0.9)),
              ),]),
      );
    }
    );
  ...
  void _buildReviewMenu(Point point) async {
      ...
      var response = await Dio().get("http://192.168.1.15:8080/comment/$latitude");
              ...
              child: ListView(
                  children: [
                    ...
                    FloatingActionButton(
                      child: const Text('SEND', textAlign: TextAlign.center, textScaleFactor: 0.9),
                      onPressed: () async {
                        var response = await Dio().post("http://192.168.1.15:8080/complain",
                            data: {'complain': complainController.text, 'latitude' : point.latitude.toString()});
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      onMapTap: (Point selectedPoint) async {
        if (addingButtonStatus == true){
          ...
          var response = await Dio().post("http://192.168.1.15:8080/mark",
              data: {'latitude' : selectedPoint.latitude, 'longitude' : selectedPoint.longitude});
      ...
      floatingActionButton: FloatingActionButton(
        ...
        child: const Text('ADD MARK', textAlign: TextAlign.center, textScaleFactor: 0.8)
      ),
    );
  }
}